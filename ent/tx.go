...
func (tx *Tx) init() {
	...
	tx.Lawyer = NewLawyerClient(tx.config)
	tx.LawyerJurisdiction = NewLawyerJurisdictionClient(tx.config)
	tx.MvpMaterial = NewMvpMaterialClient(tx.config)
	tx.MvpPlannedRoute = NewMvpPlannedRouteClient(tx.config)
	tx.MvpRoute = NewMvpRouteClient(tx.config)
	tx.MvpStaff = NewMvpStaffClient(tx.config)
	tx.MvpTruck = NewMvpTruckClient(tx.config)
	tx.PasswordToken = NewPasswordTokenClient(tx.config)
	tx.RMLUser = NewRMLUserClient(tx.config)
