...
func (dq *DecisionQuery) QueryLawyer() *LawyerQuery {
	query := (&LawyerClient{config: dq.config}).Query()
...
func (dq *DecisionQuery) Clone() *DecisionQuery {
	...
	return &DecisionQuery{
		...
		withLawyer: dq.withLawyer.Clone(),
...
func (dq *DecisionQuery) WithLawyer(opts ...func(*LawyerQuery)) *DecisionQuery {
	query := (&LawyerClient{config: dq.config}).Query()
...
func (dq *DecisionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Decision, error) {
	...
	if query := dq.withLawyer; query != nil {
		if err := dq.loadLawyer(ctx, query, nodes, nil,
			func(n *Decision, e *Lawyer) { n.Edges.Lawyer = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (dq *DecisionQuery) loadLawyer(ctx context.Context, query *LawyerQuery, nodes []*Decision, init func(*Decision), assign func(*Decision, *Lawyer)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Decision)
	for i := range nodes {
		if nodes[i].lawyer_decisions == nil {
			continue
		}
		fk := *nodes[i].lawyer_decisions
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "lawyer_decisions" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
