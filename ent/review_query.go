...
func (rq *ReviewQuery) QueryLawyer() *LawyerQuery {
	query := (&LawyerClient{config: rq.config}).Query()
...
func (rq *ReviewQuery) QueryUser() *RMLUserQuery {
	query := (&RMLUserClient{config: rq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (rq *ReviewQuery) Clone() *ReviewQuery {
	...
	return &ReviewQuery{
		...
		withLawyer: rq.withLawyer.Clone(),
...
func (rq *ReviewQuery) WithLawyer(opts ...func(*LawyerQuery)) *ReviewQuery {
	query := (&LawyerClient{config: rq.config}).Query()
...
func (rq *ReviewQuery) WithUser(opts ...func(*RMLUserQuery)) *ReviewQuery {
	query := (&RMLUserClient{config: rq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (rq *ReviewQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Review, error) {
	...
	if query := rq.withLawyer; query != nil {
		if err := rq.loadLawyer(ctx, query, nodes, nil,
			func(n *Review, e *Lawyer) { n.Edges.Lawyer = e }); err != nil {
			return nil, err
		}
	}
	if query := rq.withUser; query != nil {
		if err := rq.loadUser(ctx, query, nodes, nil,
			func(n *Review, e *RMLUser) { n.Edges.User = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *ReviewQuery) loadLawyer(ctx context.Context, query *LawyerQuery, nodes []*Review, init func(*Review), assign func(*Review, *Lawyer)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Review)
	for i := range nodes {
		if nodes[i].LawyerReviews == nil {
			continue
		}
		fk := *nodes[i].LawyerReviews
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "lawyer_reviews" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (rq *ReviewQuery) loadUser(ctx context.Context, query *RMLUserQuery, nodes []*Review, init func(*Review), assign func(*Review, *RMLUser)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Review)
	for i := range nodes {
		if nodes[i].RmlUserReviews == nil {
			continue
		}
		fk := *nodes[i].RmlUserReviews
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "rml_user_reviews" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (rq *ReviewQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := rq.ctx.Fields; len(fields) > 0 {
		...
		if rq.withLawyer != nil {
			_spec.Node.AddColumnOnce(review.FieldLawyerReviews)
		}
		if rq.withUser != nil {
			_spec.Node.AddColumnOnce(review.FieldRmlUserReviews)
		}
	}
